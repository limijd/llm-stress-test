...
| 维度 | EvalScope 引擎 | 自研引擎 |
|------|---------------|---------|
| 优势 | 社区验证、与 ModelScope 生态集成、内置数据集支持 | 零外部依赖、完全可控的指标采集、无需处理兼容性问题 |
| 劣势 | 外部依赖、版本更新可能引入不兼容、某些模型可能需要参数适配 | 需自行实现 SSE 解析和指标采集 |
| 适用场景 | 标准模型测试、需要与 evalscope 历史数据对比时 | evalscope 不可用或有兼容性问题时、需要精细控制请求参数时 |

推荐默认使用 evalscope 引擎（行业标准工具，结果可对比）。自研引擎作为 evalscope 出现兼容性问题时的备选，同时保证工具在未安装 evalscope 的环境下也能运行。长期维护策略：两个引擎共享同一套 `BaseEngine` 接口和测试用例，保持功能对等。
...
test:
  ...
  requests_per_level: [10, 50, 100, 200, 500]  # 与 concurrency 按索引一一对应，长度必须一致，不一致时 validate 报错
...
- `run()` 核心流程：
  ...
  3. 并发 POST `EngineConfig.api_url`，根据 `EngineConfig.stream` 决定流式/非流式
  4. 流式模式：解析 SSE 响应，记录首 token 时间 (TTFT)、逐 token 时间戳 (TPOT)、完成时间
     非流式模式：记录请求发出到响应完成的总延迟，TTFT 等于总延迟，token 计数从响应 usage 字段获取
  5. 汇总为 LevelResult
...
2. 按并发梯度从低到高依次执行，对每个级别：
   - 调用 engine.run()
   - metrics.aggregate() 聚合指标
   - metrics.judge() 判定通过/不通过
   - 终端实时输出本轮摘要
   - 保存本轮原始数据
   - 无论本级别是否通过，都继续执行下一级别（低级别结果仅用于报告展示）
3. 所有级别跑完后，仅以目标并发（最高级别）的结果判定整体通过/不通过
4. 目标并发通过 → 生成报告，结束
5. 目标并发不通过 → 进入降级流程
...
+-- 2026-04-10T14-30-00_DeepSeek-V3.2-Exp_evalscope/
    +-- config.yaml              # 测试配置快照（已脱敏，api_key 保留占位符或掩码）
    +-- raw/
...
- 吞吐量柱状图：X=并发数，Y=tokens/s，通过阈值虚线。单次测试生成单组柱状图；thinking=true/false 对比需分别执行两次测试，通过 report 子命令指定多个结果目录合并生成对比图（v1 不支持单次运行内自动对比）
...
包含：测试结论（最大通过并发数、建议）、通过条件判定表、性能详情表、图表、测试配置快照（已脱敏）。

### 密钥脱敏规则

所有输出产物（config.yaml 快照、summary.json、HTML 报告、日志文件）在写入前必须经过强制脱敏：
- `api_key` 字段：如果原始值是环境变量引用 `${VAR}`，保留占位符原样；如果是明文，替换为 `***REDACTED***`
- 请求/响应日志中的 `Authorization` header：掩码处理
- 脱敏在 `config.py` 中实现为 `sanitize_for_export()` 方法，报告层和日志层在输出前统一调用
...
**性能不足（正常降级）**：请求本身成功，但聚合指标不达标。正常触发降级流程。

**系统性故障（立即终止）**：认证失败(401/403)、DNS 不通、连接拒绝、连续 5xx。这类错误说明服务本身不可用，继续测试或降级毫无意义。

| 场景 | 处理 |
|------|------|
| 单请求超时/业务错误 | 记录 success=false，不中断本轮 |
| 性能不达标 | 标记 FAIL，进入降级流程（如启用） |
| 认证失败 (401/403) | **终止整个测试**，报告认证错误 |
| DNS/连接失败 | 同一轮内累计网络错误数 >= 3 → **终止整个测试** |
| 连续 5xx | 同一轮连续 10 个 5xx → **终止整个测试** |
