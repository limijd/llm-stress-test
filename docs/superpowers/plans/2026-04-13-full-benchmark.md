# Full Benchmark Auto-Test Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** A single `python3 run_full_benchmark.py --host <IP>` command that auto-detects server capabilities, runs 8 benchmark groups, and produces a delivery-grade comparison report.

**Architecture:** Two new files — `run_full_benchmark.py` (CLI entry point, ~80 lines) and `src/llm_stress_test/benchmark.py` (all logic: preflight probe, plan generation, group execution, report rendering). Composes existing `Orchestrator`, `NativeEngine`, `EvalScopeEngine`, `metrics`, `dataset` modules without modifying them.

**Tech Stack:** Python 3.10+ stdlib only (zero external dependencies). Uses existing project modules.
...
|------|--------|----------------|
| `run_full_benchmark.py` | Create | CLI entry point: argparse, call `benchmark.run()` |
| `src/llm_stress_test/benchmark.py` | Create | Preflight probes, plan generation, group execution, report rendering |

No existing files modified.

---

### Task 1: CLI Entry Point (`run_full_benchmark.py`)
...
Run: `python3 -c "import ast; ast.parse(open('run_full_benchmark.py').read()); print('OK')"`
Expected: `OK`

- [ ] **Step 3: Commit**

```bash
git add run_full_benchmark.py
git commit -m "feat: add CLI entry point for full benchmark"
```

---

### Task 2: Benchmark Module Skeleton + Preflight Health Check

**Files:**
- Create: `src/llm_stress_test/benchmark.py`

- [ ] **Step 1: Create benchmark.py with imports, helpers, and health check**
...
def _detect_slots_endpoint(base_url: str) -> int | None:
    """尝试 /slots 端点获取 parallel slots 数量。"""
...
def _detect_slots_timing(base_url: str) -> tuple[int, str]:
    """通过 timing 推断 parallel slots。返回 (S, method)。"""
    # 先试端点
...
def _check_datasets() -> tuple[bool, bool]:
    """检查 openqa 和 longalpaca 数据集。返回 (openqa_ok, longalpaca_ok)。"""
    project_root = Path(__file__).parent.parent.parent
    ds_dir = project_root / "datasets"

    openqa_ok = (ds_dir / "openqa.jsonl").exists()
    # openqa 缺失时用 fallback prompts（dataset.py 内置），也算 OK
...
def run_benchmark(
    ...
    """全自动 benchmark 主函数。返回 exit code。"""
    _echo()
    _echo(_s("╔══════════════════════════════════════════════════════════════════════╗", fg="cyan"))
...
Run: `python3 -c "import ast; ast.parse(open('src/llm_stress_test/benchmark.py').read()); print('OK')"`
Expected: `OK`

Run: `python3 -c "from src.llm_stress_test.benchmark import run_benchmark; print('import OK')"`
Expected: `import OK`

- [ ] **Step 3: Commit**

```bash
git add src/llm_stress_test/benchmark.py
git commit -m "feat: add benchmark module with preflight, execution, and reporting"
```

---

### Task 3: End-to-End Dry Run Test

**Files:**
- No new files

- [ ] **Step 1: Verify dry-run mode works locally (no server needed)**

The dry-run path should fail gracefully at health check since there's no server. Verify it doesn't crash:

Run: `python3 run_full_benchmark.py --host 127.0.0.1 --port 9999 2>&1 | head -20`
Expected: Shows banner, then "服务不可达，终止。" with exit code 1.

- [ ] **Step 2: Verify syntax of both files together**

Run: `python3 -c "import ast; ast.parse(open('run_full_benchmark.py').read()); ast.parse(open('src/llm_stress_test/benchmark.py').read()); print('All OK')"`
Expected: `All OK`

- [ ] **Step 3: Commit both files if not already committed**

```bash
git add run_full_benchmark.py src/llm_stress_test/benchmark.py
git commit -m "feat: complete full benchmark with preflight, execution, and reporting"
```

---

### Task 4: Integration Test Against Live Server

**Files:**
- No new files

This task requires the actual llama-server to be running. Replace `<TARGET_IP>` with the real IP.

- [ ] **Step 1: Run connectivity test first**

Run: `python3 test_llama_server.py <TARGET_IP> 8080`
Expected: All 3 tests pass.

- [ ] **Step 2: Run dry-run to see the plan**

Run: `python3 run_full_benchmark.py --host <TARGET_IP> --dry-run`
Expected: Shows pre-flight results (slots detected, thinking status, datasets) and full test plan table. Exits without executing.

- [ ] **Step 3: Run full benchmark with conservative override**

Run: `python3 run_full_benchmark.py --host <TARGET_IP> --max-concurrency 2`
Expected: Runs all groups with concurrency capped at 2. Each group shows progress, metrics, PASS/FAIL. Reports saved to `./results/benchmark-*/`.

- [ ] **Step 4: Verify reports**

Run: `ls ./results/benchmark-*/` and `cat ./results/benchmark-*/benchmark_report.md | head -60`
Expected: `benchmark_summary.json`, `benchmark_report.md`, `benchmark_report.html`, and per-group directories all exist. Markdown shows tables with real data.

- [ ] **Step 5: Run full auto (no cap)**

Run: `python3 run_full_benchmark.py --host <TARGET_IP>`
Expected: Auto-detects parallel slots, runs all 8 groups (or fewer if thinking/evalscope unavailable), generates full comparison report.

- [ ] **Step 6: Final commit and push**

```bash
git add -A
git commit -m "test: verify full benchmark against live llama-server"
git push
```
