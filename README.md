...
- **零依赖部署**：纯 Python 标准库实现，`python3 -m` 直接运行，无需 pip install
- **双引擎**：evalscope（行业标准）和自研 asyncio 引擎，按需切换
- **自动降级**：目标并发不通过时，自动逐步降低并发找到最大通过值
- **可配置判定条件**：Success Rate、吞吐量、TTFT 等指标，阈值自由设定
- **完整报告**：JSON/CSV 原始数据 + HTML 汇总报告（图表需可选安装 matplotlib）
- **密钥脱敏**：所有输出产物自动屏蔽 API Key
- **GUI 配置编辑器**：本地 Web UI（浏览器打开 localhost），可视化编辑配置文件，零系统依赖
...
- Python >= 3.11
- **无需 pip install** — 工具完全基于 Python 标准库，零外部依赖
...
# 直接运行，无需安装任何依赖
PYTHONPATH=src python3 -m llm_stress_test run --config config/example.yaml
```
...
```bash
# 无安装方式
PYTHONPATH=src python3 -m llm_stress_test validate --config config/my_test.yaml
...
```bash
PYTHONPATH=src python3 -m llm_stress_test run --config config/my_test.yaml
```

终端输出示例：

```
[Engine: native] 原生引擎（stdlib）可用
...
engine: "evalscope"    # "evalscope" — 调用 evalscope perf CLI
                       # "native"    — 内置 asyncio+urllib 引擎（零依赖）
...
内置引擎，纯标准库实现。用 asyncio + urllib.request 向 API 发送并发请求，解析 SSE 流式响应，精确采集每个请求的 TTFT、TPOT、延迟等指标。无需安装任何第三方包。
...
提供本地 Web UI 配置编辑器，浏览器打开即用，不依赖 Tkinter 或任何 GUI 库：
...
# 指定端口
llm-stress-config-gui --port 8080
```
...
├── _yaml.py            # 内置 YAML 子集解析器（替代 pyyaml）
